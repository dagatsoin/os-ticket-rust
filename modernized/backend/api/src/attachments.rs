...
pub async fn load_upload_policy(pool: &PgPool) -> Result<UploadPolicy, ApiError> {
    ...
    for (key, value) in rows {
        match key.as_str() {
            // Accept both the M2 storage form ("true"/"false") and the FS-032
            // settings checkbox form ("1"/"0") so an admin save via TS-M4-A1 does
            // not silently disable uploads (regression guard).
            CFG_ALLOW_ATTACHMENTS => {
                let v = value.trim();
                allow = v.eq_ignore_ascii_case("true") || v == "1";
            }
