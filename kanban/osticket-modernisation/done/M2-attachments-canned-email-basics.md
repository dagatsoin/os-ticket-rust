...
- Action: in the reply box, open the **"Canned response" dropdown** and pick **"Sample (with attachment)"** (seed id2 — the canned that carries the `policy.txt` attachment).
- Verify: the reply textarea is populated with that canned's **substituted** body (contains the real carried ticket number, no literal `%{...}` remains) AND a read-only **`policy.txt` chip** appears ("from canned response").
- Action: post the reply.
- Verify: a **NEW** agent `R` response appears reactively in the thread carrying the substituted text AND the `policy.txt` chip; the ticket badge shows **Answered**. (Note: an earlier id1 "Acknowledge receipt" reply may already exist in this continuous journey — assert against the NEW `R` entry created by this step, not the pre-existing one.)
- Status: [x]
...
- **AC-3 (US-M2-2)** failed 2026-06-11 — same drafting root cause as **TS-M2-D1 AC-2** and
  **US-M2-2 AC-3** (see their Review feedback). The root E2E journey was assembled before the
  two-sample seed clarification, so AC-3 named the variables-only canned **"Acknowledge receipt"**
  (seed id1, NO attachment) while expecting the **`policy.txt`** chip, which belongs to seed id2
  **"Sample (with attachment)"**. The implementation is correct (proven by US-M2-2 AC-3/AC-4 and
  TS-M2-D3); the tester even posted a valid substituted reply — only the chip expectation was
  unmeetable as written. **AC corrected** to select id2 "Sample (with attachment)". **No code
  change.** Marker reset to `[ ]` for re-test within the same continuous journey.
