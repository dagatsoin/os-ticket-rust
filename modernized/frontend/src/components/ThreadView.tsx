...
/** Background colors for entry types (notes have a distinct amber background per TS-M3-E3 AC-7). */
const KIND_BACKGROUND: Record<NonNullable<ThreadEntry["kind"]>, string | undefined> = {
  message: undefined,
  response: undefined,
  note: "warning.light", // Amber/yellow background for internal notes
};
        ...
        entries.map((entry) => (
          <Paper
            ...
            sx={{
              ...
              // Notes get a distinct amber/yellow background (TS-M3-E3 AC-7)
              bgcolor: KIND_BACKGROUND[entry.kind ?? "message"],
            }}
            data-testid="thread-entry"
            data-entry-kind={entry.kind ?? "message"}
          >
            <Stack
              direction="row"
            ...
            {entry.title && (
              <Typography
                ...
                data-testid="thread-entry-title"
              >
                {entry.title}
              </Typography>
            )}
