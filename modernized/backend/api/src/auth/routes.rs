...
pub async fn staff_login(
    ...
    let ip = crate::logs::client_ip(&headers);
    ...
    // TS-M4-G1: emit a Warning syslog row on a failed login attempt (best-effort),
    // rate-limited per client IP so a brute-force flood cannot grow syslog without
    // bound (WARN-3).
    let Some((staff_id, passwd, isactive)) = row else {
        log_failed_login(pool, &ip, &req.username, "Unknown username").await;
        return Err(bad());
    };
    if !isactive {
        log_failed_login(pool, &ip, &req.username, "Inactive account").await;
        return Err(bad());
    }
    let ok = verify_password(&req.password, &passwd).map_err(|_| bad())?;
    if !ok {
        log_failed_login(pool, &ip, &req.username, "Bad password for").await;
        return Err(bad());
    }

    // TS-M4-G1: emit a Debug syslog row on a successful login (best-effort).
    ost_core::log(pool, ost_core::LogType::Debug, "Staff login",
        format!("Staff #{staff_id} ('{}') logged in", req.username), ip).await;

    // FS-031.12: periodic password-reset aging. When the configured reset period
    // (months) is non-zero AND a non-admin's password age exceeds period × 30
    // days, force a password change on this login. Admins are exempt. A missing /
    // unparsable config key is treated as 0 (disabled).
    apply_password_aging(pool, staff_id).await;

    // Regeneration: minting a new session row yields a brand-new id.
    let session = store
        .create(&SessionData::Staff { staff_id, sort_prefs: None })
        .await
...
/// Write a rate-limited `Warning` syslog row for a failed staff login (WARN-3).
///
/// The write is gated by [`ost_core::note_failed_login`], a per-IP fixed-window
/// throttle: at most a bounded number of failed-login rows are written per IP per
/// window, so an unauthenticated brute-force flood cannot grow `syslog` without
/// bound (disk-fill DoS). Suppressed attempts are counted and reported on the
/// next written row (coalesced), so genuine failed-login visibility is retained.
/// Best-effort throughout — logging never blocks the (already-failing) login.
///
/// @implements WARN-3: cap attacker-driven failed-login syslog growth.
async fn log_failed_login(pool: &sqlx::postgres::PgPool, ip: &str, username: &str, reason: &str) {
    let Some(suppressed) = ost_core::note_failed_login(ip) else {
        // Over the per-window cap — drop this row (it is counted for later report).
        return;
    };
    let detail = if suppressed > 0 {
        format!("{reason} '{username}' (+{suppressed} suppressed attempt(s) since last log)")
    } else {
        format!("{reason} '{username}'")
    };
    ost_core::log(
        pool,
        ost_core::LogType::Warning,
        "Staff login failed",
        detail,
        ip.to_string(),
    )
...
/// Apply the FS-031.12 login-time password aging for one staff account.
///
/// Reads `passwd_reset_period` (months) from config; when it is a positive
/// integer and the account is a **non-admin** whose password age (since
/// `passwdreset`, or `created` for never-reset accounts) exceeds `period × 30`
/// days, sets `change_passwd = true`. Everything is best-effort: any lookup /
/// update error is swallowed so aging can never block a valid login. A missing
/// or non-positive config value disables aging (no-op).
///
/// @implements FS-031.12: forced-change on login when the password is reset-due
///   (administrators exempt); tolerant of a missing config key (0 = disabled).
async fn apply_password_aging(pool: &sqlx::postgres::PgPool, staff_id: i32) {
    let period_months: Option<i64> =
        sqlx::query_scalar::<_, String>("SELECT value FROM config WHERE key = 'passwd_reset_period'")
            .fetch_optional(pool)
            .await
            .ok()
            .flatten()
    ...
    let Some(months) = period_months.filter(|m| *m > 0) else {
        return;
    };
    ...
    let _ = sqlx::query(
        "UPDATE staff SET change_passwd = true, updated = now() \
         WHERE staff_id = $1 AND isadmin = false \
           AND COALESCE(passwdreset, created) < now() - $2::interval",
    )
    .bind(staff_id)
    .bind(&interval)
    .execute(pool)
