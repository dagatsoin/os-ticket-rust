...
- **Flow 2.9 failed** (2026-06-12) — AC-1 regression discovered during browser E2E.
  - **AC-1 (staff attaches own file)**: POST staff reply with own-file attachment returns **404 "Not Found"** error.
  - The attachment chip UI displays correctly in the staff reply composer (TS-M2-D3), but submitting the reply with an attachment fails at the backend.
  - curl-based API tests (AC-1 original) passed, but browser-driven multipart POST via the frontend fails with 404 — investigate route matching or Content-Type handling discrepancy between curl and browser fetch.
- **Flow 2.9 resolved** (2026-06-12) — the 404 was caused by a **stale Vite proxy** in the dev environment, not a code defect. After restarting the Vite dev server, the browser E2E passed. No code changes required; AC-1 re-verified and marked passing.
