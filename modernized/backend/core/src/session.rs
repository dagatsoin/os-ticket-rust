...
pub enum SessionData {
    ...
    Staff {
        ...
        /// Keys: "open", "answered", "assigned", "overdue", "closed"
        /// Values: { sort, order }
        #[serde(default, skip_serializing_if = "Option::is_none")]
        sort_prefs: Option<SortPreferences>,
    },
...
/// Per-queue sort preferences stored in the staff session (BS-020.8).
///
/// @implements BS-020.8: sticky per-queue sort preferences.
#[derive(Debug, Clone, PartialEq, Eq, Default, Serialize, Deserialize)]
pub struct SortPreferences {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub open: Option<QueueSort>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub answered: Option<QueueSort>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub assigned: Option<QueueSort>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub overdue: Option<QueueSort>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub closed: Option<QueueSort>,
}

/// A single queue's sort preference.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl SessionStore {
    ...
    /// Update the session data for an existing session.
    ///
    /// @implements BS-020.8: update session to store sort preferences.
    pub async fn update(&self, id: &str, data: &SessionData) -> Result<(), SessionError> {
        let payload = serde_json::to_string(data)?;
        sqlx::query("UPDATE session SET session_data = $1 WHERE session_id = $2")
            .bind(&payload)
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn staff_group_permissions(
        ...
        Ok(self.staff_capabilities(staff_id).await?.map(|(_, perms)| perms))
    }

    /// Load the staff member's `isadmin` flag **and** the full permission set for
    /// their group in a single query. Returns `None` when the staff id is unknown
    /// or the account is inactive (e.g. a stale session).
    ///
    /// Backs both `GET /api/staff/me` (isadmin + the four M4 capability flags,
    /// FS-032.1) and the [`crate::permission`] gate / `require_admin` helper.
    ///
    /// @implements FS-032.1: expose `staff.isadmin` + group capability flags.
    #[allow(clippy::type_complexity)]
    pub async fn staff_capabilities(
        &self,
        staff_id: i32,
    ) -> Result<Option<(bool, crate::permission::GroupPermissions)>, SessionError> {
        let row: Option<(
            bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool,
        )> = sqlx::query_as(
            "SELECT s.isadmin, g.group_enabled, g.can_create_tickets, g.can_post_reply, \
                    g.can_close_tickets, g.can_assign_tickets, g.can_transfer_tickets, \
                    g.can_delete_tickets, g.can_edit_tickets, \
                    COALESCE(g.can_manage_tickets, false), \
                    COALESCE(g.can_manage_faq, false), COALESCE(g.can_manage_premade, false), \
                    COALESCE(g.can_ban_emails, false), COALESCE(g.can_view_staff_stats, false) \
...
mod tests {
    ...
    fn session_data_round_trips_through_json_for_both_realms() {
        let staff = SessionData::Staff { staff_id: 7, sort_prefs: None };
