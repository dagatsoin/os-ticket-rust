...
async fn main() -> anyhow::Result<()> {
    ...
    let result = if reset {
        ...
        println!(
            "reset ok: purged {} → reseeded department#{} group#{} staff#{} ({} / {}) + staff#{} ({} / {}) \
             + admin staff#{} ({} / {}); orphan blobs reclaimed (seeded canned policy.txt preserved)",
    ...
    if !reset {
        println!(
            "seed ok: department#{} group#{} staff#{} ({} / {}) + staff#{} ({} / {}) + admin staff#{} ({} / {})",
            result.dept_id,
            result.group_id,
            result.staff_id,
            tools::STAFF_USERNAME,
