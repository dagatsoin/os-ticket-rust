...
pub async fn send_new_ticket_autoresponse(state: &AppState, pool: &PgPool, ticket_id: i64) {
    ...
    if let Err(err) = send_autoreply(state.mailer.active.as_ref(), mail).await {
        ...
        // TS-M4-G1: record the mailer send failure as an Error syslog row.
        ost_core::log(
            pool,
            ost_core::LogType::Error,
            "Autoresponse send failed",
            format!("Autoresponse to {to} for ticket #{ticket_id} failed: {err}"),
            String::new(),
        )
        .await;
    }
